...
describe('test register method', () => {
    ...
    test("user already registered", () => {
        ...
        register(testUser);
        expect(register(testUser)).toHaveBeenCalledWith("User is already registered!");
    })

    test("too young", () => {
        testUser.age = 10;
        register(testUser);
        expect(register(testUser)).toHaveBeenCalledWith("User is too young to register!");
    })

    test("can register", () => {
        register(testUser);
        expect(register(testUser)).toHaveBeenCalledWith("User has been registered!");
...
describe('test logIn method', () => {
    ...
    test("account details are wrong", () => {
        expect(logIn("user2", "1234")).toThrow("Username or password is incorrect.");
    })

    test("account details are correct", () => {
        expect(logIn("user1", "1234")).toHaveBeenCalledWith("Logged in successfully!");
    })
})
...
describe('test addScooter method', () => {
    ...
    test("change location and station", () => {
        ...
        addScooter("Manhattan", testScooter);
...
describe('test removeScooter method', () => {
    ...
    test("removed successfully", () => {
        ...
        expect(removeScooter(testScooter.serial)).toHaveBeenCalledWith("Scooter successfully removed!");

        removeScooter(testScooter.serial)
    ...
    test("scooter in question hasn't been added", () => {
        ...
        expect(removeScooter(testScooter.serial)).toThrow("Scooter has not been previously added!");
    })
})
